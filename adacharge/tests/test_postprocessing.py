...
class TestProjectIntoContinuousFeasiblePilots(TestCase):
    ...
    def test_rates_are_feasible(self):
        ...
        processed = project_into_continuous_feasible_pilots(rates,
                                                            self.infrastructure)
...
class TestProjectIntoDiscreteFeasiblePilots(TestCase):
    ...
    def test_rates_are_feasible(self):
        rates = np.full((5, 20), 16)
        processed = project_into_discrete_feasible_pilots(rates,
                                                          self.infrastructure)
        nptest.assert_equal(processed, 16)
...
class TestIndexBasedReallocation(TestCase):
    def setUp(self):
        self.horizon = 10
        self.sessions = session_generator(num_sessions=3,
                                          arrivals=[0] * 3,
                                          departures=[2, 3, 4],
    ...
    def test_no_reallocation_peak_binding(self):
        infrastructure = single_phase_single_constraint(num_evses=3, limit=66,
            allowable_pilots=[np.array([0, 8, 16, 24, 32]) for _ in range(3)])
        interface = TestingInterface({'active_sessions': self.sessions,
                                      'infrastructure_info': infrastructure,
                                      'current_time': 0,
                                      'period': 5})
        rates = np.full((3, self.horizon), 16)
        peak = 16*3
        processed = index_based_reallocation(rates,
    ...
    def test_reallocate_to_peak_infrastructure_not_binding(self):
        infrastructure = single_phase_single_constraint(num_evses=3, limit=66,
            allowable_pilots=[np.array([0] + list(range(8, 33)))
                              for _ in range(3)])
        interface = TestingInterface({'active_sessions': self.sessions,
                                      'infrastructure_info': infrastructure,
                                      'current_time': 0,
    ...
    def test_reallocate_infrastructure_binding_single_phase(self):
        infrastructure = single_phase_single_constraint(num_evses=3, limit=49,
            allowable_pilots=[np.array([0] + list(range(8, 33)))
                              for _ in range(3)])
        interface = TestingInterface({'active_sessions': self.sessions,
                                      'infrastructure_info': infrastructure,
    ...
    def test_reallocate_infrastructure_binding_three_phase(self):
        infrastructure = three_phase_balanced_network(evses_per_phase=1,
