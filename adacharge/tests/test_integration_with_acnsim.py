...
class AdaptiveSchedulingAlgorithmBase(TestCase):
    ...
    def test_all_rates_less_than_limit(cls):
        ...
        for ev in cls.sim.ev_history.values():
            ...
            pilot_power = cls.sim.pilot_signals[i, ev.arrival: ev.departure] * \
                          cls.sim.network.voltages[ev.station_id] / 1000 * (5/60)
            assert np.all(pilot_power <= cls.sim.network._EVSEs[ev.station_id].max_rate + eps)
            assert np.all(pilot_power <= ev.maximum_charging_power + eps)

    @classmethod
    def test_all_energy_demands_met(cls):
        assert acnsim.analysis.proportion_of_energy_delivered(cls.sim) >= .9999
...
class TestACASingleEV(AdaptiveSchedulingAlgorithmBase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.sim = acnsim.Simulator(cn, cls.alg, events, start, period=period,
                                   verbose=False)
        cls.sim.run()
...
class TestACACaltechSingleDayQuantized(AdaptiveSchedulingAlgorithmBase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.alg = AdaptiveSchedulingAlgorithm(quick_charge_obj, solver=cp.ECOS, quantize=True)
        cls.sim = acnsim.Simulator(cn, cls.alg, events, start_time, period=period, verbose=False)
        cls.sim.run()
...
class TestACACaltechOneDayUninterrupted(TestACACaltechSingleDayQuantized):
    ...
    def setUpClass(cls) -> None:
        ...
        cn = acnsim.network.sites.caltech_acn(basic_evse=basic_evse,
                                              voltage=voltage,
                                              transformer_cap=60)
        quick_charge_obj = [ObjectiveComponent(quick_charge),
                            ObjectiveComponent(equal_share, 1e-12)]

        cls.alg = AdaptiveSchedulingAlgorithm(quick_charge_obj,
                                              solver=cp.ECOS,
                                              quantize=True,
                                              uninterrupted_charging=True)
        cls.sim = acnsim.Simulator(cn, cls.alg, events, start_time,
...
def test_internal_schedule_to_schedule_output():
    ...
    alg.internal_schedule = {'PS-1': [1, 2, 3, 4, 5],
                             'PS-2': [0, 6, 7, 8, 9],
                             'PS-3': [0, 0, 0, 10, 11]}
...
class TestAdaptiveChargingAlgorithmOfflineSingleEV(AdaptiveSchedulingAlgorithmBase):
    ...
    def test_internal_schedule_matches_pilots(cls):
        assert np.allclose(cls.alg.internal_schedule['PS-1'], cls.sim.pilot_signals[0][:17], 1e-3)


class TestAdaptiveChargingAlgorithmOfflineCaltechSingleDay(AdaptiveSchedulingAlgorithmBase):
    @classmethod
    def setUpClass(cls) -> None:
        API_KEY = 'DEMO_TOKEN'
        tz = pytz.timezone('America/Los_Angeles')
    ...
    def test_internal_schedule_energy_delivered_matches_requested(cls):
        internal_energy_delivered = sum(sum(v) for v in cls.alg.internal_schedule.values()) * 208 / 1000 * (5/60)
        energy_requested = sum(ev.requested_energy for ev in cls.sim.ev_history.values())
